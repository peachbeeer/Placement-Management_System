...
public class RegisterServlet extends HttpServlet {
    ...
    private String validateInput(String name, String email, String password, String role) {
        if (name.isEmpty() || email.isEmpty() || password == null || password.isEmpty() || role.isEmpty()) return "All fields are required.";
        if (!EMAIL_PATTERN.matcher(email).matches()) return "Please enter a valid email address.";
        if (password.length() < 8) return "Password must be at least 8 characters long.";
        if (!("student".equals(role) || "admin".equals(role) || "company".equals(role))) return "Please select a valid user role.";
        return null;
    }
